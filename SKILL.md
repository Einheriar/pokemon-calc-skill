...
所有查询与伤害计算结果默认基于《宝可梦冠军》(Pokémon Champions) M-A 规则数据。当 Champions 数据与 Gen9 正作数据存在差异时，以 Champions 为准。查询结果中的 `"_data_source"` 字段标识了具体数据来源（`"champions"` 或 `"gen9"`）。
...
- `champions_*_patch.json` — 《宝可梦冠军》M-A 规则增量数据（覆盖 323 种形态，优先级高于 Gen9）
...
当 Champions 数据与 Gen9 正作数据存在差异时，以 Champions 数据为准。

### Champions 优先策略

《宝可梦冠军》是当前 VGC 对战的官方平台，本系统优先使用该数据：
- 种族值、属性、特性、招式威力等数值以 Champions M-A 规则为准
- 若查询的宝可梦/招式存在于 Champions 数据中，返回的结果将标记 `"_data_source": "champions"`
- 若 Champions 未覆盖（如部分 Gen9 宝可梦未在 Champions 中可用），自动回退到 Gen9 正作数据，标记 `"_data_source": "gen9"`

### 数据范围差异

| 维度 | Champions (M-A) | Gen9 正作 |
|------|-----------------|-----------|
| 宝可梦形态 | 323 种 | 1025+ 种 |
| 招式 | 约 776 种（已匹配） | 782 种 |
| 特性 | Champions 可用特性集 | 307 种 |

**注意**：Champions 中部分招式名称与正作存在差异，少数招式可能暂时无法匹配到正作数据库。若遇到查询不到的招式，系统会提示使用标准中文名/英文名。

### 对伤害计算的影响

伤害计算公式本身不随数据来源变化（公式由 `damage.py` 统一处理），但输入的**数值参数**可能因数据来源不同而有差异：
- Champions 数据覆盖的宝可梦：种族值、招式威力以 Champions 为准
- Gen9 回退数据：种族值、招式威力以正作数据为准

若 Champions 与 Gen9 数据存在争议（如某招式威力不同），伤害计算结果会默认使用 Champions 数值。如需对比两个版本的差异，可分别查询查看 `_data_source` 标记。
