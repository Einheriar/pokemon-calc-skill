...
2. **零外部依赖**。核心数据已静态化为 JSON，查询脚本仅使用 Python 标准库。
3. **中文优先**。数据以中文名为主索引，同时支持英文名称。
4. **能力值优先**。回答中始终使用能力值描述（如"攻击能力值 172"），而非努力值。
5. **环境参数只能通过 `field_override` 传入**。`move_override` 仅用于覆盖 `is_crit`、`hits` 等行为参数，绝对禁止修改 `base_power` 或 `type` 来模拟天气/场地效果。
