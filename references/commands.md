...
- **`--online`**：增量更新（下载队伍列表 gzip 约 1.3 MB + 仅抓取新增赛事的明细），**仅在用户显式要求最新/当前热门队伍时使用**；带 24 小时节流——距上次成功拉取不足 24 小时会直接复用现有索引、不发起请求（`meta.note` 注明）；失败回退缓存/内置数据且 `meta.online_error` 会给出失败原因，必须告知用户。
