...
滚动 30 天窗口的**全量赛事队伍**（9,000+ 支，含招式/道具/特性明细）已随仓库内置，以下查询全部开箱即用、无需联网。
...
| 按宝可梦/选手/赛事筛队伍 | "有哪些队伍带了幽尾玄鱼？" / "Sooner 最近都用什么队伍？" |
| 子集出现率统计 | "只看打进前八的队伍，谁最热门？" |
| 队友共现 | "炽焰咆哮虎最常和谁组队？" |

> 环境数据来自 [pokecamp.cc](https://pokecamp.cc) 聚合的 Limitless 公开赛事统计（滚动 30 天），内置数据离线可查，`--online` 可按需增量拉取最新数据。
